...
module.exports = {
  ...
  getAll: async (req, res) => {
    try {
      const data = await productService.merrTeGjithe();
      res.json(data);
    } catch (err) {
      res.status(500).json({ message: err.message });
    }
  ...
  getById: async (req, res) => {
    try {
      const produkti = await productService.merrSipasId(req.params.id);
  ...
  update: async (req, res) => {
    try {
      await productService.perditeso(req.params.id, req.body);
